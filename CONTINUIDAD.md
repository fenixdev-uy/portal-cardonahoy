...
Después de la grilla completa se agregó un footer exclusivo de escritorio. La corrección visual aprobable elimina todo aspecto de barra: el footer no tiene fondo diferenciado, borde completo ni sombra propia; solamente una línea interna de `1px`, con sombra mínima y márgenes laterales de `32px` a `1440px` —`24px` en PC compacto— lo separa de las cards. Debajo aparece centrado el texto **«© rsmedios.com dev en Fenix»**; únicamente **Fenix** enlaza a `https://fenixlab.uno` y abre de forma segura en una pestaña nueva. Chromium/Puppeteer verificó el símbolo, línea de `1376px` dentro de un viewport de `1440px`, fondo transparente, destino, `noopener noreferrer` y ausencia de overflow o errores. El footer permanece oculto en móvil. **No desplegado en PROD.**
