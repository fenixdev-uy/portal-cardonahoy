...
Después de la grilla completa se agregó un footer exclusivo de escritorio. La corrección visual elimina todo aspecto de barra y también el trazo sólido: el footer no tiene fondo diferenciado, borde completo ni sombra propia; la división es un relieve suave de `12px` construido con una transición transparente de sombra–luz–sombra y un desenfoque mínimo. Conserva márgenes laterales de `64px` a `1440px` —`40px` en PC compacto—. Debajo aparece centrado el texto **«© rsmedios.com dev en Fenix»**; únicamente **Fenix** enlaza a `https://fenixlab.uno` y abre de forma segura en una pestaña nueva. Chromium/Puppeteer verificó el degradado 3D, ancho de `1312px`, fondo transparente, ausencia de borde duro, destino, `noopener noreferrer`, cero overflow y consola limpia. El footer permanece oculto en móvil. **No desplegado en PROD.**
